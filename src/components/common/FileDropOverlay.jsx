  ...
  return (
    <div
      className={`pointer-events-none absolute inset-0 z-20 flex items-center justify-center ${
        isDark ? 'bg-slate-900/55' : 'bg-nas-accentSoft/40'
      }`}
      aria-hidden="true"
    >
      <div
        className={`rounded-lg border border-dashed px-5 py-3 text-center text-sm shadow-sm ${
          isDark
            ? 'border-nas-accentBorder/70 bg-slate-800/95 text-nas-accentBorder'
            : 'border-nas-accent/50 bg-white/95 text-nas-accentText'
        }`}
      >
        {message}
      </div>
