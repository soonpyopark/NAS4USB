...
export default function FileExplorer({
  ...
  const handleClearExternalCaches = async () => {
    ...
    const confirmed = await appConfirm({
      title: '캐시 정리',
      body: isExternal
        ? '원본 PDF가 없는 표시 캐시와 파일 이력을 삭제할까요? 폴더는 훑지 않고, 캐시에 적힌 경로만 확인합니다.\n\n아직 있는 파일의 하이라이트와 원본은 그대로 둡니다. 이 작업은 되돌릴 수 없습니다.'
        : `"${folderLabel}" 아래 원본이 없는 파일 이력을 삭제할까요? 폴더는 훑지 않고, 이력에 적힌 경로만 확인합니다.\n\n아직 있는 파일의 백업과 원본은 그대로 둡니다. 이 작업은 되돌릴 수 없습니다.`,
      confirmLabel: '캐시 정리',
      confirmVariant: 'danger',
    });
    if (!confirmed) return;
