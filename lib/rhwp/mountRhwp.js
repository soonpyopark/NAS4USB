...
export async function mountRhwp(container, options = {}) {
  ...
  /** @type {Set<(pointer: { px: number, py: number, visible: boolean }) => void>} */
  ...
  try {
    ...
    studio = await createRhwpStudioClient(editorHost, getStudioUrl());
    studio.onDocumentChanged(() => {
      scheduleSyncPush();
    });
    studio.onPointerChange((pointer) => {
  ...
  return {
    ...
    onPointerMove: (callback) => {
      ...
      return () => pointerListeners.delete(callback);
    },
