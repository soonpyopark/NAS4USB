...
export async function mountRhwp(container, options = {}) {
  ...
  return {
    ...
    exportPageSvgs: async () => {
      if (!studio) return [];
      const count = Number(await studio.pageCount()) || 0;
      if (count <= 0) return [];
      /** @type {{ svg: string, widthMm: number | null, heightMm: number | null }[]} */
      const pages = [];
      for (let page = 0; page < count; page += 1) {
        const svg = await studio.getPageSvg(page);
        if (!svg.trim()) continue;
        let widthMm = null;
        let heightMm = null;
        try {
          const info = await studio.getPageInfo(page);
          const width = Number(info?.width);
          const height = Number(info?.height);
          if (Number.isFinite(width) && width > 0) {
            widthMm = Math.round((width * 25.4) / 96 * 1000) / 1000;
          }
          if (Number.isFinite(height) && height > 0) {
            heightMm = Math.round((height * 25.4) / 96 * 1000) / 1000;
          }
        } catch {
          // getPageInfo is optional; SVG viewBox is enough
        }
        pages.push({ svg, widthMm, heightMm });
      }
      return pages;
    },
    setText: () => {},
