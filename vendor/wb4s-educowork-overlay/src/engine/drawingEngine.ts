...
export class DrawingEngine {
  ...
  async applyRemoteScene(
    ...
    const deferred = this.snapshotDeferredDrawState();
    const preservedPaths = this.paths.filter((path) => deferred.ids.has(path.id));
    const preservedDeletes = new Set(deferred.deletes);
    ...
    try {
      ...
      for (const path of preservedPaths) {
        if (!this.paths.some((item) => item.id === path.id)) {
          this.paths.push(structuredClone(path));
        }
      }
      if (preservedDeletes.size > 0) {
        this.paths = this.paths.filter((path) => !preservedDeletes.has(path.id));
        this.selectedIds = this.selectedIds.filter((id) => !preservedDeletes.has(id));
      }
      if (preservedPaths.length > 0 || preservedDeletes.size > 0) {
  ...
  upsertRemotePath(path: PathObject): void {
    if (this.deferredDrawPathIds.has(path.id)) return;
  ...
  removeRemotePath(pathId: string): void {
    if (this.deferredDrawPathIds.has(pathId) || this.deferredDrawPathDeletes.has(pathId)) return;
  ...
  deleteSelected(): DeleteSelectedResult | null {
    ...
    const immediatePathDeletes: string[] = [];
    for (const id of pathDeletes) {
      if (this.deferredDrawPathIds.has(id)) {
        this.deferredDrawPathIds.delete(id);
      } else {
        immediatePathDeletes.push(id);
      }
    }
    ...
    this.emitSceneWriteMany([
      ...immediatePathDeletes.map((id) => ({ type: 'path-delete' as const, id })),
    ...
    if (pathDeletes.length !== immediatePathDeletes.length) {
    ...
    if (options.tool === 'eraser' && options.eraserMode === 'stroke') {
      ...
      if (toRemove.length > 0) {
        ...
        for (const id of removeIds) {
          ...
          immediateDeleteIds.push(id);
        }

        if (immediateDeleteIds.length > 0) {
          this.emitSceneWriteMany(
            immediateDeleteIds.map((id) => ({ type: 'path-delete' as const, id })),
          );
        } else {
          this.notifyDeferredDrawChange();
        }
