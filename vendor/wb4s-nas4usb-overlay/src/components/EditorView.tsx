...
export interface EditorEmbedMode {
  ...
  userName: string;
  onReady?: (api: { exportDocument: (options?: { includeThumbnail?: boolean }) => string }) => void;
...
export function EditorView({
  ...
  const showBackButton =
    !shareLinkMode && !shareToken && (!isEmbed || Boolean(embedMode?.onClose));
  ...
  useEffect(() => {
    ...
    embedMode.onReady({
      exportDocument: (options?: { includeThumbnail?: boolean }) => {
        ...
        const thumbnail =
          options?.includeThumbnail === true
            ? generateThumbnail(paths, images, 320, 200, texts, tables)
            : docRef.current?.thumbnail;
  ...
  const commitTitle = async () => {
    ...
    if (isEmbed) {
      ...
      setTitle(next);
      if (embedMode?.onRenameTitle) {
        setSaveStatus('saving');
        try {
          await embedMode.onRenameTitle(next);
          setSaveStatus('saved');
        } catch {
          setSaveStatus('error');
          setTitle(prevTitle);
  ...
  return (
    <div className="app editor">
      <Toolbar
        ...
        onShare={handleShare}
        hideShare={false}
        backLabel={isEmbed && embedMode?.onClose ? '← 닫기' : undefined}
