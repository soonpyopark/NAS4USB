...
export function autoLaunchSupportReason() {
...
/**
 * @returns {AutoLaunchState}
 */
export function getAutoLaunchState() {
  const reason = autoLaunchSupportReason();
  if (reason) {
    return { supported: false, enabled: false, startHidden: false, execPath: process.execPath, reason };
  }
  ...
  const hidden = app.getLoginItemSettings({ path: process.execPath, args: [START_HIDDEN_ARG] });
  if (hidden.openAtLogin) {
    return { supported: true, enabled: true, startHidden: true, execPath: process.execPath, reason: '' };
  }
 ...
 * @returns {AutoLaunchState}
 */
export function setAutoLaunch(enabled, startHidden = false) {
  const reason = autoLaunchSupportReason();
  ...
  app.setLoginItemSettings({ openAtLogin: false, path: process.execPath, args: [START_HIDDEN_ARG] });
